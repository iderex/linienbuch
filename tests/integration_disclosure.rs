...
const LEGS: [&str; 6] = [
    ...
    "the_mutation_run_kills_everything_the_register_does_not_accept",
...
#[test]
#[ignore = "in the integration harness: needs cargo-mutants at the version tests/mutation/register.txt pins, which is not part of the pinned toolchain, and rebuilds and re-runs the suite once per mutant, which is minutes. It never gates a merge. Run it with: cargo test --test integration"]
